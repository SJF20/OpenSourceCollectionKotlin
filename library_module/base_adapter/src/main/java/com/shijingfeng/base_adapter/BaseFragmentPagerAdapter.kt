...
import com.shijingfeng.base_adapter.type_alias.OnFragmentCreate
import com.shijingfeng.base_adapter.type_alias.OnFragmentDestroy
import com.shijingfeng.base_adapter.util.cast
 ...
 * @author ShiJingFeng
 */
abstract class BaseFragmentPagerAdapter<T : Fragment>(
    /** FragmentManager */
    fragmentManager: FragmentManager,
    /** 是否禁止销毁   true 禁止销毁  false 系统默认销毁机制 */
    banDestroyed: Boolean = false,
    /** Fragment 创建 监听器
     *  因为 ViewPager 可以设置预先加载Fragment数量 (setOffscreenPageLimit())
     *  所以 Fragment 可能不会全部创建完, 按需添加回调监听 Fragment 创建
     */
    onFragmentCreate: OnFragmentCreate<T>? = null,
    /** Fragment 销毁 监听器 */
    onFragmentDestroy: OnFragmentDestroy? = null
) : FragmentPagerAdapter(fragmentManager, BEHAVIOR_RESUME_ONLY_CURRENT_FRAGMENT) {

    /** 是否禁止销毁   true 禁止销毁  false 系统默认销毁机制 */
    private val mBanDestroyed = banDestroyed
    /** Fragment 创建 监听器
     *  因为 ViewPager 可以设置预先加载Fragment数量 (setOffscreenPageLimit())
     *  所以 Fragment 可能不会全部创建完, 按需添加回调监听 Fragment 创建
     */
    private var mOnFragmentCreate: OnFragmentCreate<T>? = onFragmentCreate
    /** Fragment 销毁 监听器 */
    private var mOnFragmentDestroy: OnFragmentDestroy? = onFragmentDestroy
