...
//                .setAesKey()
                ...
                .setPatchLoadStatusStub((
                    ...
                    if (code == PatchStatus.CODE_LOAD_SUCCESS) {
                        Log.e("Sophix", "Sophix加载补丁成功");
                    } else if (code == PatchStatus.CODE_LOAD_RELAUNCH) {
                        // 如果需要在后台重启，建议此处用SharePreference保存状态。
                        Log.e("Sophix", "Sophix预加载补丁成功，重启应用会生效");
...
//                        SophixManager.getInstance().killProcessSafely();
                    ...
                    if (!TextUtils.isEmpty(info)) {
                        Log.e("Sophix", "补丁加载详细说明: " + info);
                    }

                    if (handlePatchVersion != 0) {
                        if (handlePatchVersion == -1) {
                            Log.e("Sophix", "补丁类型: 本地补丁");
                        } else {
                            Log.e("Sophix", "补丁类型: 后台补丁(" + handlePatchVersion + ")");
                        }
                    }
                })
